...
public class CommentBucketController {
    ...
    public Response commentDelete(@AuthenticationPrincipal User user, long commentId) {

        return new Response("message", commentService.CommentDelete(user, commentId));
    }

    @PutMapping("/{commentId}")
    public Response bucketCommentUpdate(@AuthenticationPrincipal User user, @Valid @RequestBody CommentRequestDto requestDto, long commentId) {
